...
```js
// Aici vom scrie codul nostru ZIM
const frame = new Frame("fit", 1024, 768, "white", "lightgray");
...
frame.on("ready", () => {
    ...
    const Stea = function(color) {
        ...
        this.shape.mt(0, -50);
        for (let i = 0; i < 5; i++) {
            this.shape.lt(Math.cos((18 + i * 72) / 180 * Math.PI) * 50, -Math.sin((18 + i * 72) / 180 * Math.PI) * 50);
            this.shape.lt(Math.cos((54 + i * 72) / 180 * Math.PI) * 20, -Math.sin((54 + i * 72) / 180 * Math.PI) * 20);
        }
    }
...
```js
// Adauga acest cod in continuarea celui de mai sus, in interiorul lui frame.on("ready",...)
